...
const seed = async () => {
  await connectDB();
  await Promise.all([User.deleteMany({}), Team.deleteMany({}), Player.deleteMany({}), Match.deleteMany({}), Blog.deleteMany({})]);

  // Create admin and sample user.
  const admin = await User.create({ name: 'Admin', email: process.env.ADMIN_EMAIL || 'admin@gbpl.com', password: process.env.ADMIN_PASSWORD || 'admin123', role: 'admin' });
  await User.create({ name: 'Demo User', email: 'user@gbpl.com', password: 'user123', role: 'user' });
